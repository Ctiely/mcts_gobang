...
void Board::check(Coordinate & cur) {
    unsigned int left = (cur.col - 4 > 0) ? (cur.col - 4) : 0;
    unsigned int right = (cur.col + 4 < size - 1) ? (cur.col + 4) : (size - 1);
    unsigned int top = (cur.row - 4 > 0) ? (cur.row - 4) : 0;
    unsigned int bottom = (cur.row + 4 < size - 1) ? (cur.row + 4) : (size - 1);
